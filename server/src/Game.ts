  ...
  async start() {
    ...
    while (true) {
      if (this.isGameOver()) {
        ...
        const winnerName = usernameMap.get(winner?.id ?? '') || null;
        sendMapToPlayers(`Game Over. the winner was ${winnerName}`);
        ...
        console.log(`Game Over. the winner was ${winnerName}. the losers were ${this.lossers}`);
