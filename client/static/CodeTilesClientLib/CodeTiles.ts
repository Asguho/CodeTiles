	...
	moveTowards(target: Position) {
		const isWalkable = (pos: Cords): boolean => {
			...
			if (
				x < 0 ||
				y < 0 ||
				y >= this.codeTiles.game.map.length ||
				x >= this.codeTiles.game.map[0].length
			) {
				return false;
			}
	...
	constructor(gameState: any, codeTiles: CodeTiles) {
		...
		this.base = this.map
			.reduce(
				(acc, row) =>
					acc.concat(
						row.filter((cell) => cell.type === 'base' && (cell as base).owner === this.playerId)
					),
				[] as tile[]
			)
			.find((cell) => true) as base;
