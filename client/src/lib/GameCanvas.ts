...
export function drawGame(canvas: HTMLCanvasElement, gameState: TurnData) {
	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			...
			const groundIndex = Math.floor(
				Math.random() * TileImages.grounds.length,
			);
			...
			switch (cellType) {
				...
				case "unknown": {
					const index = (x * 13 + y * 19) %
						TileImages.unknowns.length; //NOT RANDOM
			...
			const unitsAtPosition = gameState.units.filter((unit) =>
